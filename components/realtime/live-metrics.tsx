      ...
      <CardContent>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex items-center space-x-3 p-3 rounded-lg bg-blue-50">
            ...
            <div>
              <div className="text-2xl font-bold text-blue-900">{metrics.todayOrders}</div>
          ...
          <div className="flex items-center space-x-3 p-3 rounded-lg bg-green-50">
            ...
            <div>
              <div className="text-2xl font-bold text-green-900">{formatCurrency(metrics.todayRevenue)}</div>
          ...
          <div className="flex items-center space-x-3 p-3 rounded-lg bg-purple-50">
            ...
            <div>
              <div className="text-2xl font-bold text-purple-900">{metrics.activeUsers}</div>
          ...
          <div
            className={`flex items-center space-x-3 p-3 rounded-lg ${
              metrics.lowStockAlerts > 0 ? "bg-amber-50" : "bg-gray-50"
            }`}
          >
            <div className={`p-2 rounded-lg ${metrics.lowStockAlerts > 0 ? "bg-amber-100" : "bg-gray-100"}`}>
              <AlertTriangle className={`h-5 w-5 ${metrics.lowStockAlerts > 0 ? "text-amber-600" : "text-gray-600"}`} />
            </div>
            <div>
              <div className={`text-2xl font-bold ${metrics.lowStockAlerts > 0 ? "text-amber-900" : "text-gray-900"}`}>
                {metrics.lowStockAlerts}
              </div>
              <div className={`text-sm ${metrics.lowStockAlerts > 0 ? "text-amber-700" : "text-gray-700"}`}>
                Stock Alerts
              </div>
            </div>
          </div>
